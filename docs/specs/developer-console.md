...
- 작성일: 2026\. 01\. 12
- 버전: v3.1 (3-Layer Orthogonal Multi-Vector — 기저 16D / 유효 106D+)
- 대상: 외부 개발자, 고객사, 파트너사
...
#### 매칭 기능 비교 ← **v3.0 3-Tier 아키텍처**

| 매칭 기능                            | Free   | Starter | Pro        | Enterprise |
| :----------------------------------- | :----- | :------ | :--------- | :--------- |
| **Tier 1: Basic (기저 16D 코사인)**  | ✅     | ✅      | ✅         | ✅         |
...
| 기능                      | 설명                                                                                       |
| :------------------------ | :----------------------------------------------------------------------------------------- |
| **Tier 1: Basic**         | 기저 16D 벡터(L1 7D + L2 5D + L3 4D) 코사인 유사도 매칭                                    |
| **Tier 2: Advanced**      | Basic + 83개 교차축(L1×L2=35, L1×L3=28, L2×L3=20) 가중 매칭 + Extended Paradox Score       |
| **Tier 3: Exploration**   | Advanced + 정성↔정량 변환(Init/Override/Adapt/Express) + Voice 보정(±0.1)                   |
| **스마트 캐싱**           | 매칭 결과를 캐싱하여 동일 요청 시 비용 및 지연 최소화                                       |
| **커스텀 가중치**         | 고객사가 직접 교차축 가중치 테이블을 정의하고 관리                                          |
...
**Exploration Tier LLM 비용 (모든 플랜 공통):**

> ⚠️ **v3.0 변경:** 기존 "LLM 컨텍스트 분석"(선택적 애드온)은 3-Tier 매칭의 Exploration Tier로 통합. LLM 비용은 모든 플랜의 API 호출 비용에 포함.

| 항목                           | 비용             |
| :----------------------------- | :--------------- |
| Exploration Tier LLM 호출      | \~$0.00008 / call |
| 스마트 캐싱 히트 시            | $0 (캐시 재사용) |
| 예상 캐시 히트율               | 80%+             |

**실제 매칭 비용:** 캐싱 포함 시 평균 \~$0.00002 / call
...
### 8.1.4 매칭 기능 상세 ← **v3.0 3-Tier 아키텍처**

#### 8.1.4.1 3-Tier 매칭 아키텍처

DeepSight v3의 매칭 시스템은 3-Tier 다층 구조입니다 (기저 16D / 유효 106D+):

┌─────────────────────────────────────────────────────────────┐

│ Tier 3: Exploration — 정성↔정량 변환                         │

│ → Init/Override/Adapt/Express 알고리즘 + Voice 보정(±0.1)   │

│ → 모든 플랜: 기본 포함 (API 호출 비용에 포함)               │

├─────────────────────────────────────────────────────────────┤

│ Tier 2: Advanced — 교차축 가중 매칭                          │

│ → 83개 교차축 스코어 + Extended Paradox Score 반영           │
...
│ → L1(7D) + L2(5D) + L3(4D) = 기저 16D 코사인 유사도        │
...
API 요청 → Tier 판별 → Basic(기저 16D 코사인) → Advanced(교차축 보정)
...
| Tier        | 매칭 방식                                    | 응답 지연 | 모든 플랜 |
| :---------- | :------------------------------------------- | :-------- | :-------- |
| basic       | 기저 16D 코사인 유사도                        | \~5ms     | ✅        |
| advanced    | \+ 교차축 83개 가중 매칭                      | \~10ms    | ✅        |
| exploration | \+ 정성정량 변환 + Voice 보정 (기본값)         | \~50ms    | ✅        |

※ `matching_tier` 미지정 시 기본값은 `exploration`입니다. 저지연이 필요한 경우 `basic` 또는 `advanced`를 사용하세요.

#### 8.1.4.4 Free 플랜 초과 사용 방지 ← **v3.0 변경**

⚠️ **Revenue Leak 방지:** Free 플랜은 월 3,000회 호출 한도 초과 시 차단됩니다. Starter 이상 업그레이드가 필요합니다.

**검증 흐름:**

\[API 호출: use_llm_context: true\]
...
#### 8.1.4.6 매칭 퀄리티 비교 (v3 3-Tier)

| 매칭 Tier                  | 예상 정확도 | 적용 플랜      |
| :------------------------- | :---------- | :------------- |
| Tier 1: Basic (기저 16D)   | 82%         | 모든 플랜      |
| Tier 2: Advanced (교차축)  | 87%         | 모든 플랜      |
| Tier 3: Exploration (정성) | 94%         | 모든 플랜 (기본) |

※ 정확도는 내부 테스트 기준이며, 실제 환경에서 차이가 있을 수 있습니다. v3에서는 모든 플랜이 동일한 매칭 품질을 받습니다.
