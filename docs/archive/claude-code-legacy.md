...
│ │ ├── matching_service.py \# 매칭 비즈니스 로직
...
│ │ ├── persona_incubator.py \# 페르소나 자동 생성

│ │ ├── zombie_cleanup.py \# Zombie 페르소나 정리
...
CREATE INDEX idx_persona_vectors_6d ON persona_vectors USING ivfflat (vector_6d vector_cosine_ops);
...
CREATE INDEX idx_user_profiles_vector ON user_profiles USING ivfflat (vector_6d vector_cosine_ops);
...
CREATE INDEX idx_style_reviews_lookup ON style_content_reviews(style_id, content_id);
...
CREATE INDEX idx_persona_reviews_lookup ON persona_reviews(persona_id, content_id);
...
CREATE INDEX idx_matching_logs_org ON matching_logs(organization_id);
...
CREATE TABLE private_golden_samples (
...
CREATE INDEX idx_private_golden_org ON private_golden_samples(organization_id);
...
INSERT INTO domain_templates (domain, display_name, template_questions) VALUES
...
\-- v3.0: depth_high, lens_high, stance_high, scope_high, taste_high

INSERT INTO review_styles (id, depth_high, lens_high, stance_high, scope_high, taste_high, name) VALUES
...
Header: X-API-Key: ds_live_xxxxxxxx
...
GET /v1/personas/{persona_id}/reviews/{content_id}
...
\# workers/tasks/zombie_cleanup.py
...
async def validate_llm_option(organization_id: str, use_llm_context: bool, db: Session):
...
DATABASE_URL=postgresql://deepsight:password@localhost:5432/deepsight
...
API_KEY_PREFIX=ds_live\_

\# Rate Limiting

RATE_LIMIT_FREE=10 \# per minute
...
docker-compose exec api python scripts/seed_personas.py

docker-compose exec api python scripts/seed_golden_samples.py
