...
| 결정 | 내용 |
|------|------|
| 구조 | 3-Layer Orthogonal Vector System (7D + 5D + 4D = 기저 16D, 유효 106D+) |
...
기저 16D(유효 106D+) 벡터 공간에서의 랜덤 생성은 비일관적 캐릭터를 만들 위험이 높다.
...
> **핵심 인식**: 벡터 구조는 단순 7+5+4=기저 16D가 아니다 — **유효 106D+**.
...
### 14.2 `dimensions.ts` — 개별 차원 정의 (기저 16D)
...
├── dimension-colors.ts      ← 기저 16D 개별 차원 색상
...
| 0-13 | 색상 모듈 — dimension-colors.ts (기저 16D) | `src/lib/colors/dimension-colors.ts` | **신규** |
...
apps/engine-studio/src/constants/v3/dimensions.ts            ← 기저 16D 차원 정의 + 기본값
...
apps/engine-studio/src/lib/colors/dimension-colors.ts   ← 기저 16D 개별 차원 색상
