...
- [2026-03-03] **⭐ 버전업(v4.1→v4.2 등) 시 소스 파일 헤더의 버전 표기도 반드시 함께 수정할 것.** 파일 상단 `// PersonaWorld v3 — ...` 같은 헤더 주석이 실제 구현 버전과 일치하는지 확인. 특히 ① 직접 수정한 파일의 헤더 버전 ② 수정 파일이 속한 모듈 index.ts 헤더 ③ schema.prisma의 컬럼별 버전 주석 — 이 3가지를 반드시 체크. 구버전 헤더가 남으면 코드 고고학 시 혼란 유발
