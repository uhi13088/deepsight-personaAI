...
- [2026-03-12] **⭐ 새 기능(컨텍스트 주입, 상태 변경 등)을 추가하면 해당 기능이 사용되는 모든 파이프라인에 동시에 연결할 것.** chat-service만 연결하고 call-service를 빠뜨리면 통화에서는 기능이 동작하지 않음. 체크리스트: ① chat-service (채팅) ② call-service (통화) ③ kakao webhook (카카오) — 3개 파이프라인이 ConversationContext를 구성하는 모든 지점. 새 필드 추가 시 3곳 모두 업데이트 필수. (관련: T429~T432 친밀도 시스템)

## Product Understanding

- [2026-03-10] **⭐ DeepSight 벡터 시스템은 3-Layer 106D+. "16D"라는 표현 자체를 쓰지 말 것.** L1(7)+L2(5)+L3(4)=16은 기본축 수일 뿐이고, 직교 연결(Cross-Axis)하면 L1×L2=35, L1×L3=28, L2×L3=20 = 83축이 추가되어 **정량 벡터만 99D**. 여기에 정성 4D + 다이내믹스 = 106D+. "16D 정량 벡터"라는 표현도 틀림 — 정량은 99D. 표기 기준: "3-Layer Orthogonal Vector System (106D+)"
