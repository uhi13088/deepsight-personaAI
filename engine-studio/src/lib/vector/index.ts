...
export function calculateEuclideanDistance(
  v1: { depth: number; lens: number; stance: number; scope: number; taste: number; purpose: number },
...
export function calculateMagnitude(
  v: { depth: number; lens: number; stance: number; scope: number; taste: number; purpose: number }
): number {
  const dimensions = ["depth", "lens", "stance", "scope", "taste", "purpose"] as const
  let sum = 0
