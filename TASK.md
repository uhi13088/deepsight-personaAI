...
### Phase PW-V4-DB: DB 스키마 v4.0 확장 (T263~T275)

> 구현계획서 §2 + §9~12. 모든 v4.0 운영 모듈의 DB 기반.
...
- [x] **T272: KPISnapshot DB 모델 신규 생성**
  ...
  - AC3: 인덱스: snapshotType + createdAt
...
### Phase PW-V4-TYPE: 통합 타입 시스템 확장 (T276~T277)
...
- [x] **T277: 운영 타입 확장 — Quality/Moderation/Cost 타입 통합**
  ...
  - AC3: types.ts — `BudgetAlertLevel`, `CostOverrunAction`, `CostModeApplication`
  - AC4: Build PASS

### Phase PW-V4-SEC: 보안 파이프라인 통합 (T278~T286)
...
- [x] **T284: pw-kill-switch 자동 트리거 4종 구현**
  ...
  - AC3: PII Leak: 5+ PII block/24h → FREEZE_POST_GENERATION
...
- [x] **T285: pw-kill-switch → 스케줄러 기능 토글 연동**
  ...
  - AC3: pw-scheduler-service.ts — 동일 적용
...
- [x] **T286: security-middleware.ts — 유저 인터랙션 보안 미들웨어 구현**
  ...
  - AC3: inspectionLevel별 검사 강도 차등 적용 (HIGH=기본, MEDIUM=강화, LOW=심층, BLOCKED=차단)
  - AC4: 테스트 + Build PASS

### Phase PW-V4-QUAL: 품질 파이프라인 통합 (T287~T292)
...
- [x] **T292: Arena Bridge — PIS/인터뷰 기반 Arena 자동 트리거**
  ...
  - AC3: Arena 세션 자동 생성 + 관리자 알림
  - AC4: 테스트 + Build PASS

### Phase PW-V4-MOD: 모더레이션 파이프라인 통합 (T293~T298)
...
- [x] **T293: auto-moderator Stage 1+2 → post-pipeline 동기 통합**
  ...
  - AC3: QUARANTINE → PWQuarantineEntry 생성 + 포스트 미공개 저장
...
- [x] **T296: ContentReport API 라우트 — 유저 신고 접수/처리/조회**
  ...
  - AC3: `POST /api/internal/persona-world-admin/reports/[id]/resolve` — 신고 처리 (HIDDEN/DELETED/PERSONA_PAUSED/DISMISSED/NO_ACTION)
...
- [x] **T297: moderation-actions API 라우트 — 관리자 콘텐츠/페르소나/시스템 액션**
  ...
  - AC3: `POST /api/internal/persona-world-admin/moderation/system` — activateKillSwitch, deactivateKillSwitch, updateDailyBudget
  - AC4: 감사 로그 자동 기록
  - AC5: 테스트 + Build PASS
...
- [x] **T298: 관리자 모더레이션 대시보드 — 신고 큐 + 격리 큐 실 데이터**
  ...
  - AC3: PWQuarantineEntry 목록 + 승인/거부 버튼
  - AC4: Build PASS

### Phase PW-V4-COST: 비용 제어 통합 (T299~T303)
...
- [x] **T299: budget-alert → cron-scheduler-service 예산 차단 연동**
  ...
  - AC3: 100% 초과 → 포스트/댓글 생성 중단, 좋아요만 허용
  - AC4: 150% 초과 → GLOBAL_FREEZE (Kill Switch 자동 트리거)
...
- [x] **T303: optimizer 전략 3종 스케줄러 적용**
  ...
  - AC3: 캐시 히트율 최대화 — 유사 Static block 페르소나 연속 처리
  - AC4: 테스트 + Build PASS

### Phase PW-V4-API: 유저 인터랙션 전용 API 라우트 (T304~T307)

> 구현계획서 §6.5. 비즈니스 로직(like-engine, comment-engine, follow-engine, repost-engine) 완성.
...
### Phase PW-V4-ONB: 온보딩 API 확장 (T308~T314)
...
- [x] **T312: Daily Micro-Onboarding 엔진 구현**
  ...
  - AC3: `processDailyAnswer(userId, answer)` — 답변 → 벡터 미세 조정 (Adapt 알고리즘 재활용, ±0.05 클램프)
...
- [x] **T314: POST /api/persona-world/onboarding/daily-question/answer — 일일 답변 API**
  ...
  - AC3: PersonaWorldUser 벡터 업데이트 + CoinTransaction 기록
  - AC4: 테스트 + Build PASS

### Phase PW-V4-CRON: Scheduled Jobs 누락분 (T315~T320)
...
### Phase PW-V4-DI: DI Provider Prisma 구현체 (T321~T323)
...
### Phase PW-V4-FEED: 피드 v4.0 확장 (T324~T325)
...
### Phase PW-V4-STATE: PersonaState 카운터 자동 갱신 (T326)
...
- [x] **T333: test-generate DB 조회 — voiceSpec 필드 추가**
...
- [x] **T334: test-generate DB 조회 — factbook 필드 추가**
...
- [x] **T335: test-generate DB 조회 — triggerRules + demographics 필드 추가**
...
- [x] **T336: test-generate PromptBuildInput — v4 필드 매핑**
...
- [x] **T337: test-generate — PersonaState 기본값 주입**
...
- [x] **T338: PreviewTab — v4 시스템 프롬프트 표시 확인 + 버전 배지**
...
- [x] **T339: test-generate API — "call" 타입 추가**
...
- [x] **T340: test-generate API — 멀티턴 대화 지원**
...
- [x] **T341: PreviewTab — "통화" 탭 버튼 + 기본 시나리오**
...
- [x] **T344: types/api.ts — PersonaDetail에 TTS 필드 타입 추가**
...
- [x] **T345: types/api.ts — UpdatePersonaBody에 TTS 필드 타입 추가**
...
- [x] **T346: GET /api/internal/personas/[id] — TTS 5필드 응답 포함**
...
- [x] **T347: PUT /api/internal/personas/[id] — TTS 5필드 업데이트 지원**
...
- [x] **T348: OverviewTab — TTS 음성 설정 섹션 UI (provider + voiceId)**
...
- [x] **T349: OverviewTab — TTS 튜닝 UI (speed/pitch/language 슬라이더)**
...
- [x] **T350: OverviewTab — TTS 음성 프리뷰 재생 버튼**
