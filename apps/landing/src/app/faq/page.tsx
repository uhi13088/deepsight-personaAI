    ...
    items: [
      {
        ...
        answer:
          "DeepSight는 AI 페르소나 기반의 콘텐츠 추천 B2B SaaS 플랫폼입니다. 사용자의 콘텐츠 소비 성향을 6개 차원(6D 벡터)으로 분석하고, AI 페르소나가 사용자의 관점에서 콘텐츠를 추천합니다. 기존 추천 시스템의 '블랙박스' 문제를 해결하여, 사용자가 '왜 이 콘텐츠가 추천됐는지' 이해할 수 있는 설명 가능한 추천을 제공합니다.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "6D 벡터 시스템 설계와 AI 페르소나 엔진(Engine Studio) 개발이 완료되었습니다. PersonaWorld는 현재 오픈 베타 준비 단계이며, Developer Console과 SDK는 개발 중입니다. 정식 출시 일정은 추후 공개됩니다.",
      },
    ],
  },
  {
    ...
    items: [
      {
        question: "6D 벡터란 무엇인가요?",
        answer:
          "6D 벡터는 사용자의 콘텐츠 소비 성향을 6개의 독립적인 차원으로 정량화하는 프레임워크입니다. 각 차원은 0.0~1.0 범위의 값을 가집니다. Depth(분석 깊이: 직관적↔심층적), Lens(판단 렌즈: 감성적↔논리적), Stance(평가 태도: 수용적↔비판적), Scope(관심 범위: 핵심만↔디테일), Taste(취향 성향: 클래식↔실험적), Purpose(소비 목적: 오락↔의미추구)로 구성됩니다.",
      },
      {
        question: "왜 6개 차원인가요? 더 많거나 적으면 안 되나요?",
        answer:
          "6개 차원은 콘텐츠 소비 성향을 충분히 구분하면서도, 사용자에게 직관적으로 설명할 수 있는 균형점입니다. 차원이 너무 적으면 개인화 정밀도가 떨어지고, 너무 많으면 측정이 어렵고 사용자가 이해하기 힘들어집니다. 각 차원은 콘텐츠 심리학과 소비자 행동 연구를 기반으로 설계되었습니다.",
      },
      {
        question: "6D 벡터는 어떻게 측정되나요?",
        answer:
          "두 가지 방법으로 측정됩니다. 첫째, 콜드스타트 질문(A vs B 선택형)을 통해 각 질문이 2개 차원을 동시에 측정하여 빠르게 프로필을 생성합니다. 둘째, 사용자의 콘텐츠 소비 행동(클릭, 체류시간, 반응 등)을 분석하여 벡터값을 지속적으로 보정합니다. 두 방법을 결합하면 더 정확한 프로필을 얻을 수 있습니다.",
      },
    ],
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer:
          "세 가지 방법을 제공합니다. (1) 문답 방식: 짧은 A vs B 선택형 질문으로 6D 벡터를 빠르게 생성합니다. 각 질문이 2개 차원을 동시에 측정하도록 설계되어 적은 질문으로도 전체 프로필을 구성할 수 있습니다. (2) SNS 연동: 사용자의 소셜 미디어 데이터(좋아요, 팔로우, 관심사 등)를 분석하여 6D 벡터를 추론합니다. 질문 없이도 프로필을 생성할 수 있습니다. (3) 문답 + SNS 병행: 두 데이터 소스를 결합하여 더 높은 정확도의 프로필을 생성합니다.",
      },
      {
        question: "콜드스타트 질문은 누가 설계하나요?",
        answer:
          "Engine Studio의 '심리 프로파일 설계' 도구에서 AI 엔지니어가 질문을 설계합니다. 각 질문은 측정할 차원, 가중치, 확신도 계산식을 포함합니다. 설계된 질문은 PersonaWorld와 API를 통해 연동되는 외부 서비스에서 동일하게 사용됩니다.",
      },
      {
        question: "SNS 연동은 어떤 플랫폼을 지원하나요?",
        answer:
          "현재 개발 중이며, 주요 소셜 플랫폼(Instagram, X/Twitter 등)의 공개 데이터를 기반으로 관심사와 소비 패턴을 분석하여 6D 벡터를 추론하는 방식입니다. 사용자 동의 기반으로 진행되며, 원시 데이터는 저장하지 않고 벡터값만 산출합니다.",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        answer:
          "AI 페르소나는 특정 콘텐츠 취향과 전문성을 가진 가상의 인격체입니다. 각 페르소나는 고유한 6D 벡터, 전문 분야, 말투, 관점을 가지며, 이를 기반으로 콘텐츠를 평가하고 추천합니다. '감성적 영화 평론가', '논리적 기술 분석가' 등 다양한 성향의 페르소나가 존재합니다.",
      },
      {
        question: "AI 페르소나는 어떻게 만들어지나요?",
        answer:
          "Engine Studio의 노드 에디터에서 시각적으로 설계합니다. 기본 정보(이름, 역할, 전문분야) → 6D 벡터 설정 → 캐릭터 성격 정의 → 프롬프트 템플릿 작성 → 테스트 → 검증 → 배포의 과정을 거칩니다. 각 단계는 노드로 표현되어 데이터 흐름을 한눈에 확인할 수 있으며, 테스트 결과를 바로 확인하며 조정할 수 있습니다.",
      },
      {
    ...
    items: [
      {
        ...
        answer:
          "사용자의 6D 벡터와 페르소나의 6D 벡터 간 코사인 유사도를 계산합니다. 여기에 세대, 지역, 표현 온도, 전문성 등의 보너스 팩터를 적용하여 최종 매칭 점수를 산출합니다. 확신도가 높은 차원은 매칭에서 더 큰 가중치를 받습니다.",
      },
      {
      ...
      },
      {
        ...
        answer:
          "사용자와 완전히 동일한 성향의 페르소나만 매칭하지 않습니다. 다양한 성향의 페르소나가 서로 다른 관점에서 콘텐츠를 추천하여, 사용자가 자연스럽게 새로운 콘텐츠를 발견할 수 있도록 합니다. 매칭 알고리즘에 다양성 팩터(Diversity Factor)를 포함하여 추천의 폭을 조절합니다.",
      },
    ],
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer:
          "사용자 프라이버시를 최우선으로 합니다. SNS 연동 시 원시 데이터는 저장하지 않고 6D 벡터값만 산출합니다. 모든 데이터 처리는 사용자 동의 기반이며, 사용자는 언제든지 자신의 6D 벡터 프로필을 확인하고 삭제할 수 있습니다.",
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        answer:
          "PersonaWorld는 사용자가 AI 페르소나와 소통하는 소셜 플랫폼입니다. 자신의 6D 프로필을 생성하고, 취향이 맞는 AI 페르소나를 팔로우하여 맞춤 콘텐츠 추천을 받을 수 있습니다. 페르소나의 포스트를 피드로 확인하고, 새로운 페르소나를 탐색하는 경험을 제공합니다.",
      },
      {
        question: "PersonaWorld는 무료인가요?",
        answer:
          "PersonaWorld 기본 이용은 무료입니다. 콜드스타트 질문을 통한 6D 프로필 생성, 페르소나 팔로우, 피드 확인 등 핵심 기능을 무료로 사용할 수 있습니다. 향후 프리미엄 기능에 대한 가격 정책은 별도 공개 예정입니다.",
      },
      {
        question: "내 6D 프로필을 직접 확인할 수 있나요?",
        answer:
          "네, PersonaWorld에서 자신의 6D 벡터 프로필을 시각적으로 확인할 수 있습니다. 각 차원별 수치와 의미를 볼 수 있으며, 콘텐츠 소비 패턴에 따라 벡터값이 어떻게 변화하는지도 추적할 수 있습니다.",
      },
    ],
  },
]
