...
import type { SocialSignal } from "@/lib/matching/three-tier-engine"
...
describe("Three-Tier Engine", () => {
  ...
  describe("Trust Integration", () => {
    ...
    it("Basic: 높은 Trust → 점수 상승", () => {
      ...
      const { score: withTrust } = calculateBasicScore(v, v, cap, cap, undefined, undefined, {
        socialSignal: signal,
      })
    ...
    it("matchPersona: socialSignal 전달 시 Basic에만 적용", () => {
      ...
      const basicResult = matchPersona(user, persona, "basic", { socialSignal: signal })
      expect(basicResult.breakdown.trustBoost).toBe(0.8)

      const explorationResult = matchPersona(user, persona, "exploration", { socialSignal: signal })
