...
\`\`\`json
{
  "name": "Full name matching the persona's nationality/culture (e.g., Korean → 'Daniel Kwon', Japanese → 'Yuki Tanaka', Indian → 'Amir Patel')",
  "gender": "MALE | FEMALE | NON_BINARY (must match the name)",
  "handle": "Creative English SNS handle with @ prefix (e.g., '@midnight_critic', '@velvet_lens', '@chaos_curator', '@neon_dreamer'). Must reflect the persona's unique style/vibe. Use lowercase, underscores allowed, 3-20 chars after @.",
  "role": "역할 (예: '아이러니한 철학 비평가', '체계적 반역 비평가')",
  "expertise": ["전문분야1", "전문분야2", "전문분야3"],
  "description": "1인칭 자기소개 (SNS 프로필 바이오처럼, 20~60자. 예: '영화 보고 멍때리는 게 취미인 직장인')",
  ...
  "relationships": [
    {
      "type": "ally|rival|mentor|fan|confidant|frenemy|nemesis|muse|protege|crush|guardian|companion|bestie|tsundere",
...
1. name은 반드시 영어로 된 풀네임(이름+성)으로, **유저 메시지에 지정된 국적/문화권에 맞는 이름**을 사용하세요. 국적이 Korean이면 한국계 이름(예: 'Daniel Kwon', 'Luna Park'), Japanese이면 일본계(예: 'Yuki Tanaka'), Indian이면 인도계(예: 'Amir Patel') 등. 한국어 이름 금지. 이름과 성별이 반드시 일치해야 합니다.
1-1. gender는 이름에 맞는 성별 (MALE/FEMALE/NON_BINARY)
1-2. handle은 반드시 영어로, 이 페르소나의 개성/분위기를 반영한 창의적인 SNS 핸들. @로 시작, 소문자+밑줄, 3~20자. 예: '@midnight_critic', '@velvet_lens'
2. role은 벡터와 아키타입에 맞는 구체적 역할 (10~25자)
3. expertise는 3개 (이 페르소나의 전문 분야/관점)
4. description은 SNS 프로필 바이오처럼 1인칭으로 작성 (실제 사람이 쓴 것처럼. '~한 캐릭터' 같은 3인칭 표현 금지)
5. background는 구체적 디테일 포함 (장소, 사건, 인물 등). 다양한 국가/도시 배경 가능
6. speechPatterns는 3~5개 (이 캐릭터가 자주 쓰는 말투/입버릇)
7. quirks는 2~4개 (벡터 역설에서 비롯된 행동 균열)
8. habits는 2~4개 (콘텐츠 소비/생산 관련 습관)
9. relationships는 2~4개 (type은 ally/rival/mentor/fan/confidant/frenemy/nemesis/muse/protege/crush/guardian/companion/bestie/tsundere 중 하나)
10. 모든 요소가 벡터 프로파일과 일관되게 연결되어야 합니다
11. 배치 생성 시 동일한 국적/이름 패턴이 반복되지 않도록 다양성을 최우선으로 합니다`
...
function buildUserMessage(
  ...
  if (nationality || region) {
    parts.push(`## 인구통계 (반드시 반영)`)
    if (nationality) parts.push(`- 국적: ${nationality}`)
    if (region) parts.push(`- 활동 지역: ${region}`)
    parts.push(`- 이름은 반드시 이 국적/문화권에 맞는 이름으로 생성하세요`)
    parts.push(`- background(배경 서사)도 이 지역/문화와 일관되게 작성하세요`)
    parts.push("")
  }
...
function validateAndNormalize(raw: RawCharacterOutput): CharacterProfile {
  ...
  // 핸들 정규화: @prefix 보장, 소문자, 공백→밑줄
  let handle: string | undefined
  if (typeof raw.handle === "string" && raw.handle.trim().length > 0) {
    let h = raw.handle.trim().toLowerCase().replace(/\s+/g, "_")
    if (!h.startsWith("@")) h = `@${h}`
    handle = h
  }

  // LLM이 gender를 반환하면 사용, 아니면 이름에서 추론 불가 → FEMALE 기본
  const rawGender = typeof raw.gender === "string" ? raw.gender.toUpperCase() : ""
  const gender: "MALE" | "FEMALE" | "NON_BINARY" =
    rawGender === "MALE" || rawGender === "FEMALE" || rawGender === "NON_BINARY"
      ? rawGender
      : "FEMALE"
...
export async function generateCharacterWithLLM(
  ...
  const userMessage = buildUserMessage(l1, l2, l3, archetype, existingNames, nationality, region)
