...
export function validateSchema(data: unknown, schema: ValidationSchema): ValidationResult {
  ...
  for (const [field, fieldSchema] of Object.entries(schema)) {
    ...
    if (fieldErrors.length > 0) {
      ...
      sanitized[field] =
        fieldSchema.sanitize && typeof value === "string" ? sanitizeString(value) : value
    }
  }
 ...
 * 단일 필드 검증
 */
function validateField(value: unknown, field: string, schema: FieldSchema): ValidationError[] {
  const errors: ValidationError[] = []
...
export const PERSONA_CREATE_SCHEMA: ValidationSchema = {
  ...
  role: {
    type: "string",
    required: true,
    enum: ["REVIEWER", "CURATOR", "EDUCATOR", "COMPANION", "ANALYST"] as const,
  },
  vector: {
    type: "object",
    required: true,
