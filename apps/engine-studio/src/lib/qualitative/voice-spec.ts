...
/** 3-Layer 벡터에서 VoiceStyleParams 도출 (대비 스케일링 적용) */
export function computeVoiceStyleParams(
  ...
  return {
    ...
    formality: clamp(
      contrastScale(l1.lens * 0.4 + l2.conscientiousness * 0.3 + (1 - l2.extraversion) * 0.3)
    ),
    ...
    assertiveness: clamp(
      contrastScale(
        l1.stance * 0.4 +
          l2.conscientiousness * 0.2 +
          (1 - l2.agreeableness) * 0.25 +
          l1.depth * 0.15
      )
    ),
...
/** 중앙(0.5) 기준으로 대비를 강화하여 값 분산을 넓힘 */
export function contrastScale(value: number, factor: number = 1.8): number {
  const centered = (value - 0.5) * factor
  return 1 / (1 + Math.exp(-centered * 4))
}
