...
const BLOCKED_WORDS = ["욕설1", "욕설2", "비속어1", "비속어2", "금지어1"]
...
export default function SafetyFiltersPage() {
  ...
  const handleThresholdChange = (id: string, value: number[]) => {
    setFilters(filters.map((f) => (f.id === id ? { ...f, threshold: value[0] } : f)))
  }
  ...
  const handleBulkAddWords = () => {
    ...
    const words = bulkWords
      .split("\n")
      .map((w) => w.trim())
      .filter((w) => w && !blockedWords.includes(w))
    if (words.length === 0) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{FILTER_STATS.avgResponseTime}ms</div>
            <p className="text-muted-foreground mt-1 text-xs">필터 처리</p>
          </CardContent>
        </Card>
      </div>
                    ...
                    <div className="mb-4 flex items-start justify-between">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-muted-foreground mt-1 text-sm">{filter.description}</p>
                        </div>
                      </div>
                    ...
                    {filter.enabled && (
                      <div className="grid gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
                            <Label>임계값</Label>
                            <span className="font-mono text-sm">{filter.threshold.toFixed(2)}</span>
                          </div>
                          <Slider
