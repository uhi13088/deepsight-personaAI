...
export function TraitColorBar({
  ...
  return (
    <div className={`flex flex-col ${config.gap}`}>
      {/* 라벨 행 */}
      {showLabels && (
        <div className={`flex items-center justify-between ${config.text} text-gray-500`}>
      ...
      <div className="relative">
        ...
        <svg
          ...
          viewBox="0 0 10 10"
        >
          <polygon points="5,10 0,2 10,2" fill="#374151" />
        </svg>
      </div>

      {/* 퍼센트 값 */}
      {showValue && (
        <div className={`text-center ${config.text} font-semibold text-gray-700`}>
          {percentage}%
        </div>
      )}
    </div>
