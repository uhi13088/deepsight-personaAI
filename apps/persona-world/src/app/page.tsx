...
export default function LoginPage() {
  ...
  const { profile, setProfile, completeOnboarding, reset, restoreFollows, restoreActivity } =
    useUserStore()
  ...
  const registerGoogleUser = useCallback(
    async (email: string, name: string | null | undefined, image: string | null | undefined) => {
      ...
      try {
        ...
        if (json.success && json.data) {
          ...
          // 서버에서 팔로우/활동 목록 복원 (reset으로 초기화된 경우 대비)
          void restoreFollows()
    ...
    [isRegistering, setProfile, restoreFollows, restoreActivity, router]
  )
